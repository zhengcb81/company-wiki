...
def _decode(data: bytes) -> tuple[str, bool]:
    for encoding in ("utf-8-sig", "utf-8", "gb18030"):
        try:
            return _nfc_lf(data.decode(encoding)), encoding not in {"utf-8-sig", "utf-8"}
...
def _text_markdown(
    ...
    for result in results:
        parts.extend((f"<!-- locator: {result.coordinates.locator()} -->", result.raw_text or "", ""))
    return _Normalized(
        body="\n".join(parts).rstrip() + "\n",
            ...
            for table in found_tables:
                try:
                    ...
                    if table_bbox is None or not callable(markdown_method):
                        raise ValueError("table geometry or Markdown API is unavailable")
...
def _docling_markdown(path: Path, source_id: str) -> _Normalized:
    ...
    for table in document.tables:
        ...
        result = _parser_result(
            ...
            parser_name="dayu_docling",
            parser_version=parser_version,
            structured_value={"kind": "table", "page_number": page_number, "table_index": table_index},
        )
...
def _html_text_markdown(text: str, source_id: str, *, format_name: str, repaired: bool) -> _Normalized:
...
def _mht_markdown(path: Path, source_id: str) -> _Normalized:
    ...
    return _text_markdown(path, source_id, parser_name="mht_text", parser_version="1.0.0")
...
def _docx_markdown(path: Path, source_id: str) -> _Normalized:
    ...
    for paragraph in document.paragraphs:
        ...
        raw.append(value)
    for table in document.tables:
        rows = [[_nfc_lf(cell.text).strip() for cell in row.cells] for row in table.rows]
        if rows:
...
def _markdown_table(rows: list[list[Any]]) -> str:
    ...
    if width == 0:
        return ""
    normalized = [[_escape_cell(row[index] if index < len(row) else "") for index in range(width)] for row in rows]
    header = normalized[0]
...
def _xlsx_markdown(path: Path, source_id: str) -> _Normalized:
    ...
    for sheet_index, sheet in enumerate(workbook.worksheets):
        ...
        raw = "\n".join(" | ".join(_escape_cell(value) for value in row) for row in rows)
    ...
    return _Normalized("\n".join(parts), tuple(results), "openpyxl", str(openpyxl.__version__), "completed", ())
...
def _pptx_markdown(path: Path, source_id: str) -> _Normalized:
    ...
    for slide_index, slide in enumerate(deck.slides):
        ...
        raw = "\n\n".join(values)
        parts.extend((f"## Slide {page_number}", "", f"<!-- locator: loc:v1/page:{page_number} -->", "", raw, ""))
        results.append(
            _parser_result(
                source_id=source_id,
                raw_text=raw,
    ...
    return _Normalized("\n".join(parts), tuple(results), "python_pptx", str(pptx.__version__), "completed", ())
...
def _doc_markdown(path: Path, source_id: str) -> _Normalized:
    ...
    if completed.returncode != 0:
        raise ValueError("antiword failed: " + completed.stderr.decode("utf-8", errors="replace")[:300])
...
def _unsupported(path: Path, reason: str) -> _Normalized:
    ...
    return _Normalized(body, (), "unsupported_format", "1.0.0", "unsupported", ("unsupported_format",), reason)
...
def _normalize_source(
    ...
    if suffix in {".txt", ".md", ".csv"}:
        return _text_markdown(path, source_id, parser_name="plain_text", parser_version="1.0.0")
...
def normalize_catalog(
    ...
    *,
    ...
    for document_index, document in enumerate(documents, start=1):
        ...
        primary = next((item for item in locations if item["role"] == "original_primary" and item["source_id"] == source_id), None)
        if primary is None:
            failed += 1
            continue
        ...
        if source_path.suffix.lower() == ".pdf" and expected_pdf_sha == document["content_sha256"]:
            sidecar = next((item for item in locations if item["role"] == "processed_docling"), None)
        ...
        try:
            ...
            normalized = _unsupported(source_path, f"{type(exc).__name__}: {str(exc)[:500]}")
            bundle = IngestService(root=Path(primary["root_path"])).ingest(
        ...
        content = _frontmatter(document, normalized) + f"# {document['title']}\n\n" + normalized.body
        _atomic_write(output_path, content)
        artifact_hash = _sha256_file(output_path)
        artifact_id = "urn:company-wiki:artifact:sha256:" + hashlib.sha256(
            (document["document_id"] + "\0normalized\0" + NORMALIZER_VERSION).encode("utf-8")
        ).hexdigest()
        with store.transaction() as connection:
            connection.execute("DELETE FROM evidence_spans WHERE document_id=?", (document["document_id"],))
        ...
        if normalized.status == "completed":
            ...
            unsupported += 1
    return ProcessingReport("normalize", completed, skipped, partial, unsupported, failed)


__all__ = ["normalize_catalog"]
