...
def _parser() -> argparse.ArgumentParser:
    ...
    scan.add_argument("--dry-run", action="store_true", help="count candidates without creating catalog files")
    scan.add_argument("--root-id", action="append", help="scan only this configured root; repeatable")

    normalize = subparsers.add_parser("normalize", help="create normalized Markdown and EvidenceSpans")
    normalize.add_argument("--limit", type=int)
    normalize.add_argument("--force", action="store_true")

    summarize = subparsers.add_parser("summarize", help="create deterministic source-only summaries")
    summarize.add_argument("--limit", type=int)
    summarize.add_argument("--force", action="store_true")

    subparsers.add_parser("export", help="export documents.csv, artifacts.csv, and index.md")
    ...
    query = subparsers.add_parser("query", help="query catalog metadata and artifact paths")
    ...
    extraction_identity = extraction_quality.add_mutually_exclusive_group(
        required=True
    )
    ...
    duplicates.add_argument("--text", help="filter by company, title, kind, date, or path")
    ...
    run = subparsers.add_parser("run", help="scan, normalize, summarize, and export in order")
    ...
    worker.add_argument("--once", action="store_true", help="run one scheduling cycle and exit")
    worker.add_argument(
    ...
    install = subparsers.add_parser(
        "install-startup", help="install a Windows logon task without starting it immediately"
    )
    install.add_argument("--task-name", default=DEFAULT_TASK_NAME)

    uninstall = subparsers.add_parser("uninstall-startup", help="remove the Windows logon task")
    uninstall.add_argument("--task-name", default=DEFAULT_TASK_NAME)

    startup = subparsers.add_parser("startup-status", help="show Windows logon task status")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    def identify_company() -> IdentityResult:
        ...
        if args.security_id and args.security_id.casefold() != result.resolved.security_id.casefold():
    ...
    def source_request(*, allow_download: bool = False) -> tuple[SourceRequest, IdentityResult | None]:
    ...
    try:
        if args.command == "scan":
            ...
            if args.refresh:
                ...
                refresh = OfficialSecurityMasterRefresher(store).refresh(markets=markets)
            ...
            stage = str(result.get("worker_status") or ("waiting" if live else "stopped"))
            if stage == "idle":
                stage = "waiting"
            ...
            if not args.once and worker_controller().status()["desired_state"] == "paused":
            ...
            result = {
                ...
                "normalize": get_catalog().normalize(limit=args.limit, force=args.force),
                "summarize": get_catalog().summarize(limit=args.limit, force=args.force),
