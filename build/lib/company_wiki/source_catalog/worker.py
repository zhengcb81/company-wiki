...
def load_worker_config(path: Path, *, project_root: Path) -> WorkerConfig:
    ...
    if set(payload) != expected:
        raise ValueError(
            f"worker config must contain exact fields; missing={sorted(expected - set(payload))}, "
...
class SourceCatalogWorker:
    ...
    def _append_log(self, payload: dict[str, Any]) -> None:
        ...
        with self.log_path.open("a", encoding="utf-8", newline="\n") as stream:
            stream.write(json.dumps(_plain(payload), ensure_ascii=False, sort_keys=True) + "\n")

    @staticmethod
    def _processed_count(report: Any) -> int:
        return int(getattr(report, "completed", 0)) + int(getattr(report, "partial", 0)) + int(
            getattr(report, "unsupported", 0)
        )
    ...
    def run_cycle(
        ...
        *,
        ...
        def begin_stage(
            stage: SourceOnlyStage, *, detail: str | None = None
        ) -> None:
            if activity is None:
                return
        ...
        scan_due = last_scan is None or timestamp - float(last_scan) >= self.config.scan_interval_seconds
        export_due = self.state["last_export_at"] is None or timestamp - float(
            self.state["last_export_at"] or 0
        ...
        if eligible:
            ...
            # Kept as a compatibility alias for existing log/status consumers.
            ...
            if retry_after is None or timestamp >= float(retry_after):
                ...
                begin_stage(
                    summarize_stage, detail="selecting next Markdown document"
                )
                try:
                    ...
                    failure_scope = getattr(result["summarize_llm"], "failure_scope", None)
                    if failed:
                        self.state["last_error"] = str(
                            getattr(result["summarize_llm"], "error", None)
                            or "LLM summary batch failed"
                        )
                    if failed and failure_scope != "document":
            ...
            export_due = export_due or normalized > 0 or summarized > 0 or (
        ...
        if summary_result is not None:
            deferred_or_failed = isinstance(summary_result, dict) and summary_result.get(
                "status"
            ) in {"failed", "deferred"}
    ...
    def run_forever(
        ...
        *,
        ...
        if control.status().get("desired_state") == "paused":
        ...
        with session:
            ...
            if startup_delay_seconds > 0 and not session.wait(startup_delay_seconds):
