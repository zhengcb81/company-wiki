...
class DayuCliDownloadAdapter:
    ...
    def discover(self, request: SourceRequest) -> tuple[DownloadCandidate, ...]:
        ...
        if completed.returncode != 0:
            detail = (completed.stderr.strip() or completed.stdout.strip())[-3000:]
            self.close()
            raise DayuCliAdapterError(
                f"Dayu CLI exited {completed.returncode}: {detail or 'no output'}"
            )
        try:
    ...
    def _candidate_from_meta(
        ...
        if value.get("ingest_complete") is not True or value.get("is_deleted") is True:
            return None
