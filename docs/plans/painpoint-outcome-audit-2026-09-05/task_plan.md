...
## Phase 7：六类遗留痛点实施细化 — in_progress（2026-09-08，仅文档）

- [x] 在既有R4下展开H01、WP02–10、FC-903的操作顺序、输入产物、失败恢复与独立验收（初稿88子步骤，待独立复核）。
- [ ] 建立117个原ID逐行修复/验收规划映射，保留原审计结论及原证据入口，明确当前结果待取证。
- [x] 显式映射9个架构泄漏/7类真实验收，明确旧95门的历史地位和R4实际执行顺序。
- [ ] 独立复核新增文档，核ID/链接/核心历史hash与未授权写入边界；更新入口和接班记录。

## Next Step

完成117逐项映射并独立复核两份新文档，保持原R4两份被审核心文档及历史签署字节不变。
