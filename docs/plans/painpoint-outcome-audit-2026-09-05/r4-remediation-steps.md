...
| H01风险控制 | A.DR后D01–D04可准备，与B设计并行；禁止回收的运行资格先证明所有自动入口不可达 | risk_contained；不等同archive_restore_verified或允许prune |
...
| FC903/资格修复 | 只读历史取证可先做；隔离资格机制可先测；新业务签收等相关真实结果 | 历史绑定有效性与当前业务资格分别记录，不阻无关query/open |
