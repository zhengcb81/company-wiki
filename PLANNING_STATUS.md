...
> ⚠️ **唯一未定边界 S-7**：`config.py` 46/46、`scanner.py` 140/140、`policy.py` 5/5 **恰好顶格**（FC-1204 复杂度棘轮默认运行）→ 若某步无法做到**复杂度中性**，是否允许**更新棘轮表**（= 改既有测试文件，与 F10"仅新增"互斥）？**作者建议不允许**（把新判定放进新增模块/函数）。
