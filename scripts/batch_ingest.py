...
def is_high_value(file_path: str) -> bool:
    ...
    if re.search(r'\d{4}年?度?年度报告(?!摘要|审计|内部控制|鉴证|专项)', n):
        return True
    if re.search(r'\d{4}年?半年度?报告(?!摘要)', n) or re.search(r'\d{4}年?半年报(?!摘要)', n):
        return True
    ...
    if '投资者关系' in n or '调研' in n or '接待' in n:
        return True
    # 重大事件
    if any(k in n for k in ['并购', '收购', '定增', '增发', '股权激励', '重大资产重组', '预案']):
        return True
    return False


def scan_company_files(company_name, ingested_set=None, skip_ingested=True, high_value_only=False):
    """扫描某公司的所有待处理文件"""
...
def main():
    ...
    parser.add_argument("--high-value-only", action="store_true", help="只处理高价值文件")
        ...
        try:
            ...
            if status == "success":
                ...
                if result.get("assessment_updated"):
                    total_assessments += 1
                print(f"  -> OK | entries:{result.get('entries_added',0)} assess:{result.get('assessment_updated',False)}")
                mark_ingested(fp)
            elif status == "skip":
                skipped += 1
    ...
    print("-" * 50)
    print(f"Done. Entries:{total_entries} Assessments:{total_assessments} Skip:{skipped} Err:{errors}")
